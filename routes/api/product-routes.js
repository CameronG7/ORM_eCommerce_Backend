  ...
  Product.create(req.body)
    .then((product) => {
      ...
      if (req.body.tagIds.length) {
        console.log(typeof JSON.parse(req.body.tagIds));
        const productTagIdArr = JSON.parse(req.body.tagIds).map((tag_id) => {
          // have to parse the stringified array
          return {
  ...
  Product.update(req.body, {
    ...
    .then(async (productTags) => {
      ...
      if (req.body.tagIds) {
        ...
        // figure out which tags to remove based if they don't match the newProductTags
        const productTagsToRemove = productTags
          .filter(({ tag_id }) => !JSON.parse(req.body.tagIds).includes(tag_id))
          .map(({ id }) => id);
